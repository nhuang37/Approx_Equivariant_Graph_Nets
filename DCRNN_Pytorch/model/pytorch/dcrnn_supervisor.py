...
class DCRNNSupervisor:
    def __init__(self, adj_mx, aut_flag, orbits, idx_swap_back, **kwargs):
        self._kwargs = kwargs
        ...
        #self.decay = self._train_kwargs.get('decay', 1.)
        ...
        self.gc_flag = bool(self._model_kwargs.get('use_gc_for_ru', True)) #default True

        # setup model
        dcrnn_model = DCRNNModel(adj_mx, aut_flag, orbits, idx_swap_back, self.gc_flag, self._logger, **self._model_kwargs)
    ...
    def _train(self, base_lr,
        ...
        wait = 0
        optimizer = torch.optim.Adam(self.dcrnn_model.parameters(), lr=base_lr, eps=epsilon) #, weight_decay=self.decay)

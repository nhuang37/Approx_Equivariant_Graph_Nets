...
from models.sem_gcn_autG import SemGCNautG
from models.sem_gcn_triv import SemGCNtriv
import json
...
def parse_args():
    ...
    #different symmetry choices
    parser.add_argument('--no_tie',action='store_false',  help='same scalar on diagona/same off-diagonal, default to use (else, tie per node)')
    parser.add_argument('--aut',action='store_true',  help='use the aut-G equivariant semGCN')
    parser.add_argument('--triv',action='store_true',  help='preserve no equivariance')
    #strict or approx equiv layer
    parser.add_argument('--no_gc',action='store_false',  help='use graph conv A in aut_layer; default to use')
    parser.add_argument('--no_ew',action='store_false',  help='use learnable edge weights M in aut_layer; default to use')
    parser.add_argument('--pointwise',action='store_true',  help='use A as pointwise masking f equivariant map; default false')
    #subset of train
...
def main(args):
    ...
    if args.aut:
        left_index = dataset.skeleton()._joints_left
        right_index = dataset.skeleton()._joints_right
        singletons = [x for x in list(range(16)) if x not in left_index and x not in right_index]
        pairs = [(left, right) for left, right in zip(left_index, right_index)]
        model_pos = SemGCNautG(adj, args.hid_dim, pairs, singletons, num_layers=args.num_layers,
                                   p_dropout=p_dropout, gc_flag=args.no_gc, ew_flag=args.no_ew, 
                                   pt=args.pointwise).to(device)
    
    elif args.triv:
        model_pos = SemGCNtriv(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout,
                                gc_flag=args.no_gc, ew_flag=args.no_ew, pt=args.pointwise).to(device)        
    else:
        model_pos = SemGCN(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout,
                       gc_flag=args.no_gc, ew_flag=args.no_ew, pt=args.pointwise, tie_all=args.no_tie).to(device)
        # model_pos = SemGCN(adj, args.hid_dim, num_layers=args.num_layers, p_dropout=p_dropout, gc_flag=args.no_gc, ew_flag=args.no_ew,
    ...
    if args.resume or args.evaluate:
        ...
        arg_file = path.join(ckpt_dir_path, 'args.txt')
    ...
    train_loader = DataLoader(PoseGenerator(poses_train, poses_train_2d, actions_train, args.subset), batch_size=args.batch_size,
